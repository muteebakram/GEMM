  ...
  if ((i < Ni) && (j < Nj))
  {
    ...
    for (int ks = 0; ks < Nk; ks += 16 * BLOCK_SIZE)
    {
      for (int m = 0; m < 16; m++)
      {
        ...
        sA1[ty][tx + m * BLOCK_SIZE] = A[(ks + m * BLOCK_SIZE + tx) * Ni + i];
        sB1[ty + m * BLOCK_SIZE][tx] = B[j * Nk + ks + ty + m * BLOCK_SIZE];
      }
      __syncthreads();
  ...
  for (int ks = 0; ks < ks_end; ks++)
  {
    ...
    for (int m = 0; m < TILE; m++)
    {
      ...
      if (i < Ni && (km_jump + tx < Nk))
        sA[ty][tx + m_jump] = A[(km_jump + tx) * Ni + i];
      else
        sA[ty][tx + m_jump] = 0;
